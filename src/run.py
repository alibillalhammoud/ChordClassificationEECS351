...
subparsers = parser.add_subparsers(
    required=True,
    help="Choose to input data in real time from the mic OR from a WAV file",
    dest="inputmode",
)

parser_realtime = subparsers.add_parser(
    "mic", help="sets the input method to real time input from the microphone"
)
parser_realtime.add_argument(
    ...
    type=float,
    default=2,
    help="set the audio buffer length in seconds",
    required=False,
)
parser_realtime.add_argument(
    "-t",
    ...
    type=float,
    default=0,
    help="set how long in seconds you want the program to run. 0 results in looping forever until ctrl-c is pressed",
    required=False,
)
parser_realtime.add_argument(
    "-w",
    ...
    action="store_true",
    default=False,
    help="writes all input data collected from the mic to a wav file.",
)
parser_realtime.add_argument(
    "-n",
    "--noisefilter",
...
parser_file.add_argument(
    "-s", "--src", type=argparse.FileType("r"), help="src file (wav format)", required=True,
)
