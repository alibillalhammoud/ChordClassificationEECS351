...
def do_spectrogram(samples, sample_rate, window_length, padding_factor, padtype):
    ...
    frequencies, times, spectrogram = signal.spectrogram(
        samples, window=hamming_window, fs=sample_rate, nfft=dft_length, noverlap=0
    )
    ...
    for j in range(spect_cols):
        for i in range(spect_rows):
            for h in harmonic_orders:
                if (input_spectrogram[i, j] > magn_threshold) and (
                    input_spectrogram[math.floor(1.0 * i / h), j] > magn_threshold
                ):
...
def do_harmonic_correction(input_spectrogram, harmonic_threshold, cut_amount, harmonic_order):
    return np.clip(
        input_spectrogram
        - cut_amount
        * get_harmonic_spectrogram(input_spectrogram, harmonic_threshold, harmonic_order),
        0,
        1,
    )
    ...
    for j in range(spect_cols):
        for i in range(spect_rows):
            if spectrogram[i, j] > magn_threshold:
                ...
                if (
                    not [times[j], freq2note(frequencies[i], note_frequencies)]
                    in detected_notes_list
                ):
                    detected_notes_list.append(
                        [times[j], freq2note(frequencies[i], note_frequencies)]
                    )
