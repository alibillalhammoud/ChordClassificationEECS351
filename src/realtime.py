...
class ProcessingCallback:
    ...
    class NoiseFilter:
        ...
        def __init__(self, mode: str):
            if not isinstance(mode, str):
                raise TypeError("NoiseFilter mode must be specified as a string")
            if mode not in ["dynamic", "linear", "attenuated", "none"]:
                raise NotImplementedError("NoiseFilter only support dynamic or linear modes")
            self.__memory_type = mode
            self.__current_volume_benchmark = []

        def update_volume_benchmark(self, most_recent_volume: float):
            if self.__memory_type == "none":
                return
            if not isinstance(float(most_recent_volume), float):
                raise TypeError("most_recent_volume must be number like")
            if not self.__current_volume_benchmark:
                self.__current_volume_benchmark.append(most_recent_volume)
            elif self.__memory_type == "dynamic":
                self.__current_volume_benchmark.append(most_recent_volume)
                if (
                    len(self.__current_volume_benchmark)
                    > ProcessingCallback.NoiseFilter.dynamic_memory_length
                ):
                    self.__current_volume_benchmark.pop(0)
            elif (
                self.__memory_type == "linear"
                and self.__current_volume_benchmark[0] < most_recent_volume
            ):
    ...
    def __init__(self, device_sample_rate, record=False, memtype="dynamic"):
        if not isinstance(record, bool):
            raise TypeError("bad arg ProcessingCallback constructor, record must be bool")
        if not isinstance(int(device_sample_rate), int):
            raise TypeError("bad arg ProcessingCallback constructor, sample rate must be number")
    ...
    def __call__(self, input_data, frame_count, time_info, status):
        ...
        if self.__tstart is None:
            self.__tstart = (
                time.time() - self.__srate * frame_count
            )  # estimate time instead of use pyaudio time
        if status:
        ...
        try:
            ...
            print_chord(
                detected_notes, toffset=time.time() - self.__srate * frame_count - self.__tstart,
            )
...
def listenANDprocess(
    ...
    if not isinstance(deviceInfo, AudioDeviceInfo):
        raise TypeError("bad arg deviceInfo listenANDprocess func")
    if not isinstance(float(buffersize), float):
        raise TypeError("bad arg buffersize listenANDprocess func")
    if not isinstance(float(listentime), float):
        raise TypeError("bad arg listentime listenANDprocess func")
    if not isinstance(record, bool):
        raise TypeError("bad arg record listenANDprocess func")
    if not isinstance(noisefilter, str):
        raise TypeError(
            "bad arg noisefilter mode must be specified as a string listenANDprocess func"
        )
    # create audio stream
    ...
    # begin processing
    micstream = micinst.open(
        format=deviceInfo.format,
        channels=deviceInfo.channels,
        rate=deviceInfo.device_sample_rate,
        input=True,
        output=False,
        frames_per_buffer=buffersamps,
    ...
    try:
        if listentime != 0:
            while (time.time() - timeout_counter_start) < listentime:
                pass
        else:
            while True:
                pass
